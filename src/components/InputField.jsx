...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    ...
    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(99,102,241,0.4)] border-[var(--primary-color)]"
      : "border-white/20 hover:border-[var(--primary-color)]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[var(--primary-color)]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[var(--primary-color)] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[var(--primary-color)]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
